	...
	private void initialize(int length, int height) {
		...
		for (int x = 1; x < points.length-1; ++x) {
			for (int y = 1; y < points[x].length-1; ++y) {
				...
				/*
				points[x][y].neNeighbor = points[x+1][y+1];
				points[x][y].nwNeighbor = points[x-1][y+1];
				points[x][y].seNeighbor = points[x+1][y-1];
				points[x][y].swNeighbor = points[x-1][y-1];
